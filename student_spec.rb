...
class TestLibrary < MiniTest::Test
  ...
  def test_book_list
    assert_equal(@books, @our_library.books)
  end
  def test_find_book
    assert_equal(@books[1], @our_library.find_book("fake_book"))
  end
  def test_find_rental
    assert_equal(@books[1][:rental_details], @our_library.find_rental("fake_book"))
  end
  def test_add_book
    @our_library.add_book("new_book")
    result = @our_library.find_book("new_book")
    assert_equal(@books[3], result)
  end
